...
int umnet_socket(int domain, int type, int protocol){
	
	int ret = umnet_msocket(NULL, domain, type, protocol);
	printf("umnet_socket #%d -> ",ret);
	switch(domain){
		case AF_LOCAL:
...
static void __attribute__ ((constructor)) init (void)
{
	...
	//SERVICESOCKET(s, socket, umnet_socket);
